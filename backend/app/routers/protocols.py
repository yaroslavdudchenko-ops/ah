...
async def update_protocol(
    protocol_id: str, body: ProtocolUpdate, db: AsyncSession = Depends(get_db)
):
    protocol = await _get_or_404(protocol_id, db)
    ...
    db.add(AuditLog(
        entity_type="protocol", entity_id=protocol_id,
        action="update", metadata_={"fields": list(update_data.keys())},
    ))
    await db.flush()
    await db.refresh(protocol)
    return protocol
...
async def delete_protocol(protocol_id: str, db: AsyncSession = Depends(get_db)):
    protocol = await _get_or_404(protocol_id, db)
    db.add(AuditLog(
        entity_type="protocol", entity_id=protocol_id, action="delete", metadata_={},
    ))
    await db.delete(protocol)


@router.get("/{protocol_id}/versions", response_model=list[VersionResponse])
async def list_versions(protocol_id: str, db: AsyncSession = Depends(get_db)):
...
async def get_version(protocol_id: str, version_id: str, db: AsyncSession = Depends(get_db)):
    await _get_or_404(protocol_id, db)
    result = await db.execute(
...
# P2 STUB: diff between versions
@router.get("/{protocol_id}/diff")
async def diff_versions(protocol_id: str, v1: int, v2: int, db: AsyncSession = Depends(get_db)):
    """P2 feature — diff between two versions (not implemented in MVP)."""
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail=error_body(
            "NOT_IMPLEMENTED",
            "Version diff is a P2 feature. Planned for next release.",
        ),
    )
